...
from .utils import (OMAGIC, TMAGIC, ConfigParserError, DirectoryStorageError,
                    DirectoryStorageVersionError, FileDoesNotExist, logger,
                    loglevel_BLATHER, oid2str, timestamp2tid, z64, z128)
...
class BaseDirectoryStorage(BaseStorage):
    ...
    def __init__(
        self, filesystem, read_only=0, synchronous=0, argv=_some_unique_object
    ):
        ...
        if classname != self.__class__.__name__:
            raise DirectoryStorageError(
                "Wrong classname, %r!=%r" % (classname, self.__class__.__name__)
            )
        ...
        self.history_timeout = self.filesystem.config.getint(
            "storage", "history_timeout"
        )
        self.delay_delete = self.filesystem.config.getint("storage", "delay_delete")
        #
        try:
            self.min_pack_time = self.filesystem.config.getint(
                "storage", "min_pack_time"
            )
            ...
            logger.info(
                "[storage]/min_pack_time is missing, "
                "using [storage]/window_size instead"
            )
        #
        try:
            self.check_dangling_references = self.filesystem.config.getint(
                "storage", "check_dangling_references"
            )
        except ConfigParserError:
            # settings files from 1.0 alpha 1 or earlier do not have this
            logger.info(
                "assuming config/settings should have "
        ...
        try:
            ...
            logger.info(
                "assuming config/settings should have " "[storage]/keep_policy=detailed"
            )
    ...
    def _check_object_file(self, oid, serial, data, check_md5):
        ...
        if l != len(data):
            raise DirectoryStorageError(
                "Wrong length of file for oid %r, %d, %d" % (oid2str(oid), l, len(data))
            )
        appoid = data[8:16]
        if oid != appoid:
            raise DirectoryStorageError(
                "oid mismatch %r %r" % (oid2str(oid), oid2str(appoid))
            )
        md5sum = data[40:56]
        ...
        if md5sum != z128 and check_md5:
            if hashlib.md5(serials_plus_pickle).digest() != md5sum:
                raise DirectoryStorageError(
                    "Pickle checksum error reading oid %r" % (oid2str(oid),)
                )
        appserial = serials_plus_pickle[8:16]
        if serial is not None and serial != appserial:
            raise DirectoryStorageError(
                "serial mismatch %r %r in oid %r"
                % (oid2str(appserial), oid2str(serial), oid2str(oid))
            )
    ...
    def _make_file_body(self, oid, serial, old_serial, data, undofrom=z64):
        header = OMAGIC + struct.pack("!I", len(data) + 72) + oid + undofrom + z128
        ...
        try:
            if self._do_packing_in_new_thread:
                ...
                if "exc_info" in mydict:
                    ...
                    raise mydict["exc_info"][0](mydict["exc_info"][1]).with_traceback(
                        mydict["exc_info"][2]
                    )
                else:
                    return mydict["r"]
    ...
    def _pack_impl(self, referencesf, t):
        ...
        if t > upper_limit:
            ...
            logger.log(
                loglevel_BLATHER,
                "pack time threshold moved back " "by %d seconds" % (t - upper_limit),
            )
            t = upper_limit
        t = timestamp2tid(t)
        if t > self._prev_serial.encode() and self.min_pack_time > 0:
            ...
            logger.log(
                loglevel_BLATHER,
                "pack time threshold moved back to " "date of last write transaction",
            )
        if t > self._last_pack:
            ...
            self.filesystem.write_file(
                "A/" + self.filesystem.filename_munge("x.packed"), t
            )
        else:
