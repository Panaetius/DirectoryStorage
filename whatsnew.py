...
from .utils import ConfigParser, oid2str, tid2date
...
def main():
    ...
    if verbose >= 0:
        print(
            "Newest transaction is",
            oid2str(current_tid),
            tid2date(current_tid),
            file=sys.stderr,
        )
    ...
    while limit_tid < current_tid:
        ...
        transaction_filename = os.path.join(
            "A", filename_munge(_tid_filename(current_tid))
        )
        files += 1
    ...
    if verbose >= 1:
        print(
            "%d files in %d objects in %d transactions"
            % (files, len(oids), transactions),
            file=sys.stderr,
        )
