...
from DirectoryStorage.utils import (CMAGIC, OMAGIC, TMAGIC, ConfigParser,
                                    DirectoryStorageError, FileDoesNotExist,
                                    ZODB_referencesf, class_name_from_pickle,
                                    oid2str, timestamp2tid, z64, z128)
...
class CoreChecker(BaseChecker):
    ...
    def sanity(self):
        if self.verbose >= 0:
            print(
                "Basic sanity-checking of the directory structure...", file=self.output
            )
        ...
        for file in ["misc", "B", "journal"]:
            if not self.filesystem.exists(file):
                self.panic(
                    "directory %r is missing (solution; create an empty one)" % (file,)
                )
...
class FullChecker(BaseChecker):
    ...
    def check_history(self, tid):
        ...
        while 1:
            ...
            if nexttid > tid:
                self.problem(
                    "transaction file with backwards next-transaction pointer", name
                )
                break
            tid = nexttid
            if tid == z64:
    ...
    def traverse(self, oid):
        ...
        if total_length:
            if self.verbose >= 0:
                print(
                    "%10d maximum referencee graph depth" % (max_length,),
                    file=self.output,
                )
                print(
                    "%10.1f average reference graph depth" % (total_length / float(n),),
                    file=self.output,
                )
            ...
            if fs.exists(tname):
                if tid < "\x03F%\xa0\x00\x00\x00\x00":
                    ...
                    if not self.mc.is_marked(tname):
                        self.problem(
                            "data files whose transaction file is not linked into history",
                            name,
                        )
            else:
    ...
    def check_object_file(self, oid, serial, data, name):
        ...
        if data[24:40] != z128:
            self.problem(
                "object data files with non-zero bits in the reserved area", name
            )
            return 1
        md5sum = data[40:56]
