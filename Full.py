...
from ZODB import POSException, TimeStamp
from ZODB.ConflictResolution import ConflictResolvingStorage

from .BaseDirectoryStorage import BaseDirectoryStorage
from .utils import (CMAGIC, OMAGIC, TMAGIC, DanglingReferenceError,
                    DirectoryStorageError, DirectoryStorageVersionError,
                    FileDoesNotExist, POSGeorgeBaileyKeyError,
                    ZODB_referencesf, class_name_from_pickle, logger, oid2str,
                    timestamp2tid, z16, z64, z128)
...
class Full(BaseDirectoryStorage, ConflictResolvingStorage):
    def _load_object_file(self, oid, serial=None):
        ...
        try:
            data = self.filesystem.read_database_file(
                "o" + oid2str(oid) + "." + oid2str(serial)
            )
    ...
    def store(self, oid, serial, data, version, transaction):
        if self._is_read_only:
            raise POSException.ReadOnlyError(
                "Can not store to a read-only DirectoryStorage"
            )
    ...
    def restore(self, oid, serial, data, version, prev_txn, transaction):
        ...
        if self._is_read_only:
            raise POSException.ReadOnlyError(
                "Can not restore to a read-only DirectoryStorage"
            )
        if transaction is not self._transaction:
            raise POSException.StorageTransactionError(self, transaction)
        if version:
    ...
    def _vote_impl(self):
        ...
        body = (
            struct.pack("!HHHIH", len(u), len(d), len(e), len(ob), 0) + u + d + e + ob
        )
        if self._md5_write:
    ...
    def undoLog(self, first=0, last=-20, filter=None):
        ...
        while i < last:
            ...
            while oidblock:
                ...
                try:
                    odata = self.filesystem.read_database_file(
                        "o" + stroid + "." + strtid
                    )
                except FileDoesNotExist:
                    if tid >= self._last_pack:
                ...
                if prevtid == z64:
                    ...
                    try:
                        podata = self.filesystem.read_database_file(
                            "o" + stroid + "." + strprevtid
                        )
                    except FileDoesNotExist:
    ...
    def _check_transaction_file(self, tid, data, check_md5):
        ...
        if TMAGIC != data[:4]:
            raise DirectoryStorageError(
                "Bad magic number in transaction id %r" % (strtid,)
            )
        apptid = data[8:16]
        if tid != apptid:
            raise DirectoryStorageError(
                "tid mismatch %r %r" % (strtid, str2oid(apptid))
            )
        l = struct.unpack("!I", data[4:8])[0]
        if l != len(data):
            raise DirectoryStorageError(
                "Wrong length of file for tid %r, %d, %d" % (strtid, l, len(data))
            )
        md5sum = data[32:48]
        vdata = data[48:]
        if md5sum != z128 and check_md5:
            if hashlib.md5(vdata).digest() != md5sum:
                raise DirectoryStorageError(
                    "Pickle checksum error reading oid %r" % (stroid,)
                )
    ...
    def transactionalUndo(self, transaction_id, transaction):
        ...
        if self._is_read_only:
            raise POSException.ReadOnlyError(
                "Can not undo in a read-only DirectoryStorage"
            )
        if transaction is not self._transaction:
            raise POSException.StorageTransactionError(self, transaction)
        ...
        while oidblock:
            ...
            if prevtid == z64:
                ...
                # compute a new file
                body = self._make_file_body(
                    oid, this_transaction, current, data[72:], undofrom=prevtid
                )
                self._write_object_file(oid, this_transaction, body)
        return list(oids.keys())

    def undo(self, transaction_id, transaction):
        return self.get_current_transaction(), self.transactionalUndo(
    ...
    def _pack(self, t, referencesf):
        if not self._has_root():
            logger.log(self.filesystem.ENGINE_NOISE, "Skipping pack of empty storage")
        ...
        logger.info(
            "Packing complete, removed %d files, elapsed time %s" % (total, elapsed)
        )
        ...
        for file_ in fs.listdir(directory):
            ...
            if file_.endswith("-deleted"):
                ...
                if time_deleted + self.delay_delete < now:
                    if pretend:
                        print(
                            "packing would delay-remove %r" % (path,), file=sys.stderr
                        )
                    else:
                        fs.unlink(path)
            else:
...
def _fix_serial(data, oid):
    if len(data) == 8:
        ...
        if data[:4] != CMAGIC:
            raise DirectoryStorageError(
                "Bad magic number in oid pointer %r" % (oid2str(oid),)
            )
        return data[4:12]
    else:
